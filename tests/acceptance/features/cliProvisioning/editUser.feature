...
Feature: edit users
  ...
  Scenario Outline: Admin resets user password with special characters
    ...
    And user "brand-new-user" should exist
    When user "brand-new-user" uploads file with content "some text" to "atextfile.txt" using the WebDAV API
    Then the content of file "atextfile.txt" for user "brand-new-user" using password "<password>" should be "some text"
